...
Astar::Astar(GridMap* map) : PathFind_base::PathFind_base(map)
{
...
std::vector<Pixel> Astar::computePath(Pixel start, Pixel end)
{
   //spwap start and end
   _st = end;
   _en = start;
   ...
   while(!astar_rdy)
   {
      ...
      for(unsigned int i = 0; i < childs.size(); ++i)
      {
         ...
         {//update f(x) and parent ptr
            ...
            astar_f_type g = currNode->g + (childs[i].isDiagonal ? _cost_long_step : _cost_short_step);
...
void Astar::getChildNodes(std::vector<Astar_child>& vec, Pixel node)
{
...
void Astar::tracePath(Astar_node* node)
{
   if(node == NULL)
   {
      return;
